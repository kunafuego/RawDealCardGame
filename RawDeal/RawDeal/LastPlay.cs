...
public class LastPlay
{
    ...
    public override string ToString()
    {
        if(LastPlayPlayed != null) 
            return
            $"LastPlayPlayed: {Convert.ToString(LastPlayPlayed.Card)} Was it played on the same turn: {WasItPlayedOnSameTurnThanActualPlay} IrishWhip: {WasThisLastPlayAManeuverPlayedAfterIrishWhip}";
        return "";
    }
}