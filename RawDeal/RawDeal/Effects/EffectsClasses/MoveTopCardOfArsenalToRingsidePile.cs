...
public class MoveTopCardOfArsenalToRingsidePile : Effect
{
    ...
    {
        _view.SayThatPlayerDamagedHimself(playerThatPlayedCard.GetSuperstarName(),
            AmountOfDamageToMake);
        _view.SayThatSuperstarWillTakeSomeDamage(playerThatPlayedCard.GetSuperstarName(),
            AmountOfDamageToMake);
        CheckIfGameIsFinished(playerThatPlayedCard);
        playerThatPlayedCard.MoveArsenalTopCardToRingside();
        var cardThatWentToRingside = playerThatPlayedCard.GetCardOnTopOfRingside();
        _view.ShowCardOverturnByTakingDamage(cardThatWentToRingside.ToString(),
            AmountOfDamageToMake,
            AmountOfDamageToMake);
    }
